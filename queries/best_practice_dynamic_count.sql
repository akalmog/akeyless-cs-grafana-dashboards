-- Headline stat: dynamic + rotated secret count from latest objects report.
...
SELECT
    SUM(CASE WHEN ro.object_type LIKE '%dynamic_secret%' THEN ro.amount ELSE 0 END)
    + SUM(CASE WHEN ro.object_type LIKE '%rotated_secret%' THEN ro.amount ELSE 0 END) AS value
