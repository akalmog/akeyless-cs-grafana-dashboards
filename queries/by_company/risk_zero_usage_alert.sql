-- Month-over-month usage status — one row per customer + product.
...
Ranked AS (
    SELECT
        mpu.*,
        ROW_NUMBER() OVER (PARTITION BY mpu.company_name, mpu.product ORDER BY mpu.report_month DESC) AS rn
    FROM CompletedMonths mpu
),
Latest AS (
    SELECT company_name, product, report_month, used_clients
    FROM Ranked
    WHERE rn = 1
),
Previous AS (
...
LEFT JOIN Latest curr ON curr.company_name = ca.company_name AND curr.product = p.product
LEFT JOIN Previous prev ON prev.company_name = ca.company_name AND prev.product = p.product
