-- Best practice: authentication comparisons aggregated by customer name.
...
SELECT
    ar.company_name AS "Customer",
    (SELECT GROUP_CONCAT(DISTINCT account_id)
     FROM CompanyAccounts ca_ids
     WHERE ca_ids.company_name = ar.company_name) AS "Account IDs",
...
FROM AuthRows ar
ORDER BY ar.company_name, ar.comparison
