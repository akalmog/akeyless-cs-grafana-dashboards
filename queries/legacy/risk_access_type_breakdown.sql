-- Access type usage by product (long format; omits zero-usage product rows).
...
GROUP BY c.name, c.account_id, rc.access_type, lrd.latest_report_date, rcp.product
HAVING SUM(rcp.amount) > 0 OR SUM(rcp.exceeded_amount) > 0
ORDER BY c.name, "Used Clients" DESC, "Product"
