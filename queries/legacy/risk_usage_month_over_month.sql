-- Month over Month usage change (same dedup as SM/SRA/PM "Used" stat panels).
...
MonthlyProductUsage AS (
    ...
    GROUP BY mlr.account_id, mlr.report_month, mlr.product, mlr.latest_report_date
),
Ranked AS (
    SELECT *,
        ROW_NUMBER() OVER (PARTITION BY account_id, product ORDER BY report_month DESC) AS rn
...
LEFT JOIN Ranked prev
    ...
    AND prev.rn = 2
INNER JOIN companies c ON c.account_id = curr.account_id
WHERE curr.rn = 1
ORDER BY c.name, curr.product
