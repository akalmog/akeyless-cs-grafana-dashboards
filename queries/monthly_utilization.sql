-- Monthly utilization % using deduplicated latest ObjectsReport per month.
...
AccountHubSpotData AS (
    SELECT ca.account_id, c.{purchased_col}, c.current_contract_start_date
    FROM CompanyAccounts ca
    INNER JOIN companies c ON ca.account_id = c.account_id
    WHERE c.name = '${company_name}'
),
PurchasedTotal AS (
    SELECT CAST(COALESCE({purchased_col}, '0') AS INTEGER) AS purchased_total
